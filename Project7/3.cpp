...
int main(int argc, char *argv[])
{
    ...
    if (me == BOSS) {
        for (int dst = 0; dst < NumCpus; dst++) {
            if (dst == BOSS) continue;
            // MPI_Send(array, numToSend, type, dst, tag, MPI_COMM_WORLD);
            MPI_Send(&BigSignal[dst * PPSize], PPSize + MAXSHIFTS, MPI_FLOAT, dst, TAG_SCATTER,
                     MPI_COMM_WORLD);
        }
    } else {
        // MPI_Recv(array, maxCanReceive, type, src, tag, MPI_COMM_WORLD, &status);
        MPI_Recv(PPSignal, PPSize + MAXSHIFTS, MPI_FLOAT, BOSS, TAG_SCATTER, MPI_COMM_WORLD,
                 &status);
    }
